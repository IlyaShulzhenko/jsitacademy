let formDef1 = [
  ...
  array.forEach(item => {
    item.classList.add("arraysClass");
});
  
    array.forEach(item => {
    item.addEventListener('input', validation);
    item.addEventListener('blur', validation);
 });

    select.addEventListener('blur', validation);
    select.addEventListener('change', validation);
    ...
    function validation(){
        ...
        if ('type') {
            ...
            addClassErrorOrGood.apply({type, value}, ['good', 'error']);
            removeMessageError.call({type, value});
        } else {
            addClassErrorOrGood.apply({type, value}, ['error', 'good']);

if('type' === 'text'){
    messageError.call({type, value}, 'Ошибка, введите верное значение');
}
if('type' === 'number'){
  messageError.call({type, value}, 'Ошибка, введите верное значение');
}
...
function addClassErrorOrGood(add, remove) {
...
function isText(value){
    const line = '';
    return value.math(line);
}
function isNumber(value){
  const numb = '';
  return value.math(numb);
}
...
const arCLass = document.querySelectorAll('.arraysClass');

form1.addEventListener('submit', event);

function event(value) {
    const arsClass = [...arCLass].filter(arraysClass => !arraysClass.value);
    arsClass.forEach(arCLass => {
        const { type } = arCLass;

        const parentArCLass = arCLass.parentElement;
        if (!parentArCLass.querySelector('.error')) {
          const error = type 
        if(type === 'text'){
          createError('Ошибка, введите верное значение')        
        } else if( type === "number"){
          createError('Ошибка, введите верное значение')
        } else if(type === "checkbox"){
          createError('Ошибка, введите верное значение')
